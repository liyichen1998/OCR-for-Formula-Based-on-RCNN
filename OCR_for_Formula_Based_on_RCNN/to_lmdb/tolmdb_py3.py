...
def init_args():
    ...
    args.add_argument('-v',
                      '--val_dir',
                      type=str
                      , help='The generated mdb file val dir',
                      default='small')
    args.add_argument('-m',
                      '--map_size',
...
if __name__ == '__main__':
    ...
    num = 0
    for line in lines:
        imgPath = os.path.join(imgDir, str(num)+'.png')#line.split()[0].decode('utf-8')
        print(imgPath)
        num += 1       
        word = line.split()[0].decode('utf-8')
        print(word)
        if num<=10000 :
            val_imgPathList.append(imgPath)
            val_labelList.append(word)
        else :
            imgPathList.append(imgPath)
            labelList.append(word)
        
    createDataset(args.save_dir, imgPathList, labelList, args.map_size)
    createDataset(args.val_dir, val_imgPathList, val_labelList, args.map_size)
