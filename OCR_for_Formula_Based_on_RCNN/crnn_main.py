...
def init_args():
    ...
    args.add_argument('--valroot', help='path to dataset', default='./to_lmdb/val')
...
if __name__ == '__main__':
    ...
    # read train set
    train_dataset = dataset.lmdbDataset(root=args.trainroot, transform=dataset.resizeNormalize((192, 64)))
    assert train_dataset
    if not params.random_sample:
        sampler = dataset.randomSequentialSampler(train_dataset, params.batchSize)
    else:
        sampler = None
    # images will be resize to 60*180
    ...
    # read test set
    # images will be resize to 60*180
    test_dataset = dataset.lmdbDataset(root=args.valroot, transform=dataset.resizeNormalize((192, 64)))
    ...
    # cnn and rnn
    image = torch.FloatTensor(params.batchSize, 3, params.imgW, params.imgH)
